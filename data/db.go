...
func ExecSQLWithoutConn(cfg DBConfig, sql string) error {
	if len(sql) == 0 {
		return nil
	}
...
type DataScan func(rows *sql.Rows) (interface{}, error)

func ExecSQLWithReurn(cfg DBConfig, sql string, scan DataScan) []interface{} {
	if len(sql) == 0 {
		return nil
	}
	...
	for rows.Next() {
		dest, err := scan(rows)
		if err != nil {
			log.Error(" Convert struct faield", zap.Error(err))
			continue
		}
